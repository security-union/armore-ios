//
//   ArmoreTests.swift
//   ArmoreTests
//
//  Created by Dario Talarico on 1/2/20.
//  Copyright © 2020 Security Union. All rights reserved.

import XCTest
@testable import Armore

class RescueLinkTests: XCTestCase {

    override func setUp() {
        // Put setup code here. This method is called before the invocation of each test method in the class.
    }

    override func tearDown() {
        // Put teardown code here. This method is called after the invocation of each test method in the class.
    }
    func testPasswordEncryption() {
        let passwordToEncrypt = "coche"
        let encryptedPassword = passwordToEncrypt.sha256Base64()
        XCTAssertEqual(encryptedPassword, "j7Oqot5F4CHAqOh/RIpEGN1VJ5pny7rdYHIX/3FJ+T4=")
    }
    func testAcceptInvitationURL() {
        let urls = URLs()
        let urlToTest = urls.acceptInvitation(withId: "invitation-id-32253")
        XCTAssertEqual(urlToTest, "http://localhost:20000/invitations/invitation-id-32253/accept")
    }

    func testRejectInvitationURL() {
        let urls = URLs()
        let urlToTest = urls.rejectInvitation(withId: "invitation-id-32253")
        XCTAssertEqual(urlToTest, "http://localhost:20000/invitations/invitation-id-32253/reject")
    }

    func testCancelInvitationURL() {
        let urls = URLs()
        let urlToTest = urls.cancelInvitation(withId: "invitation-id-32253")
        XCTAssertEqual(urlToTest, "http://localhost:20000/invitations/invitation-id-32253/cancel")
    }

    func testImageWithIdURL() {
        let urls = URLs()
        let urlToTest = urls.imageWithId(imageId: "image-id-2797")
        XCTAssertEqual(urlToTest, "http://localhost:20000/image/image-id-2797")
    }
    
    //  swiftlint:disable line_length
    func testImageGetBase64() {
        let imageToTest: String = UIImage(named: "close")?.getBase64() ?? ""
        // swiftlint:disable:next_line_length
        XCTAssertEqual(imageToTest, "")
    }
    //  swiftlint:enable line_length
    
    // method to test that two user objects are the same
    func sameUserObjects(u1: User, u2: User) -> Bool {
        if u1.username != u2.username {
            return false
        }
        if u1.email != u2.email {
            return false
        }
        if u1.firstName != u2.firstName {
            return false
        }
        if u1.lastName != u2.lastName {
            return false
        }
        if u1.pictureURL != u2.pictureURL {
            return false
        }
        return true
    }    
    
}
